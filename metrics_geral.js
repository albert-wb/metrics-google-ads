...
function main() {
  ...
  // Define o intervalo "Todo o Período" (Desde 2000 até Hoje)
  var todayYMD = Utilities.formatDate(new Date(), timeZone, 'yyyy-MM-dd');
  var dateCondition = `segments.date BETWEEN '2000-01-01' AND '${todayYMD}'`;
  
  // 1. Pega dados Gerais e Detalhados (Vitalício)
  ...
  var subject = `[${config.accountLabel}] Relatório VITALÍCIO: ${accountName} - Até ${dateString}`;
  ...
  pdfBlob.setName(`Relatorio_Vitalicio_${config.accountLabel}_${dateString.replace(/\//g, '-')}.pdf`);
  ...
  Logger.log("Relatório Vitalício enviado para " + config.email);
}
...
function buildHtmlReport(accountName, gen, devices, ages, campaigns, dateString) {
  ...
  var html = `<div style="font-family: Arial, sans-serif; color: #333;">
    <h2 style="font-family: Arial; border-bottom: 2px solid #4285f4; padding-bottom: 10px;">
      <span style="background-color: #34a853; color: white; padding: 2px 8px; border-radius: 4px; font-size: 0.8em; vertical-align: middle;">LIFETIME</span>
      ...
      Relatório: ${accountName}
    </h2>
    <p style="font-size:12px; color: #666;">
       <b>Período:</b> Todo o Histórico (Até ${dateString})<br>
  ...
  // 1. KPI CARDS
  html += `<h3>📊 Visão Geral (Vitalício)</h3>
  ...
  // 2. VIDEO
  html += `<h3>📹 Engajamento Médio (Histórico)</h3>
  <table style="${styleTable}">
  ...
  // 3. TABELA DETALHADA
  html += `<h3>🚀 Desempenho por Campanha (Vitalício)</h3>
  <table style="${styleTable}">
    <tr style="background-color: #e8eaed;">
        <th style="${styleTh}">Campanha</th>
        <th style="${styleTh}">Custo Total</th>
  ...
  if (campaigns && campaigns.length > 0) {
      ...
      html += `<tr><td colspan="5" style="${styleTd}">Nenhuma campanha encontrada.</td></tr>`;
  }
  html += `</table>`;
